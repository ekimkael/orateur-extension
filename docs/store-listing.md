...
Aucun compte, aucun traceur, aucune donnée collectée. Le texte que vous écoutez
sur la page ne quitte jamais votre machine. Rien n'est envoyé vers Orateur sans
que vous cliquiez pour le demander.
...
- Ne collecte **aucune** des catégories proposées.
...
- `data_collection_permissions: { required: ["none"] }` est déjà déclaré dans le
